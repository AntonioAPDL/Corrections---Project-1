...
In addition, we benchmark all Bayesian models against raw physical forecast ensembles (ECMWF/GloFAS and NOAA/NWM).
...
For transparency, the table below reports the mean CRPS over the forecast window for every model and every cutoff used in this validation exercise (bold indicates the lowest CRPS within each cutoff column). The model labels follow the notation defined above.
...
\toprule
Model label & 20210123 & 20211112 & 20211221 & 20220511 & 20221225 \\
\midrule
...
The main validation message is clear. First, multivariate synthesis models with transfer function active during the forecast window (AL-M-T1 and exAL-M-T1) provide the strongest and most consistent CRPS performance; exAL-M-T1 is strictly best in three cutoffs (and tied in one), while AL-M-T1 is strictly best in one cutoff (and tied in one). Second, incorporating climate-institution retrospective and ensemble information (multivariate vs univariate formulations) yields substantial gains, showing that these products contain predictive signal that can be effectively exploited by the synthesis framework. Third, in all five cutoffs, the best corrected model improves on the best raw physical ensemble baseline.

This revision addresses this concern directly: the paper no longer relies only on internal variants of the proposed method. It includes simpler Bayesian alternatives (NDLM and AL-DQLM) and raw physical ensembles as explicit benchmarks, so readers can assess the practical gain from additional model complexity.
