...
| Supplementary material | `472+` | parameter summaries, long-cycle figure, univariate transfer-active synthesis, setup panels |
...
| HE-5 code/reproducibility | `done_pending_final_archive_doi` | Manuscript code availability `443-447`; response HE-5; article software manifest `artifacts/software_availability/software_availability_manifest.json`; article software contract `docs/software_availability_contract.md`; workflow contract `repro/run/REVISION_SOFTWARE_REPRODUCIBILITY_CONTRACT_20260615.md`; workflow release-readiness files `README.md`, `CITATION.cff`, `RELEASE_NOTES_PENDING_FINAL_ARCHIVE.md`, and `docs/workflow_archive_readiness_20260615.md` | The revised manuscript and response now cite the CRAN `exdqlm` package, the CRAN package DOI, and the public workflow repository. The workflow archive DOI is intentionally pending until final revision freeze; citation metadata and release notes therefore describe a pending final archive rather than a completed archived release. | Before final resubmission, confirm the workflow license, create the permanent workflow release/archive, replace `pending` DOI fields, and re-run the cross-repo validators and compiles. |
| HE-6 out-of-sample validation | `done` | Manuscript data-role and rolling-origin paragraphs; response HE-6; article `artifacts/forecast_design/forecast_design_manifest.json`; workflow `docs/he6_out_of_sample_forecast_design_contract_20260615.md` | The revised article and response now explicitly distinguish fit-time inputs, forecast-generation inputs, and held-out post-cutoff USGS verification. Fitting uses USGS observations and retrospective products through the cutoff; forecast generation uses latest issued forecast products plus forecast-window PPT/SOIL covariates from the staged origin bundle. The canonical GDPC factor is treated as a deterministic climate-index covariate rather than an operational forecast product. | Keep the compact forecast-design manifest, workflow contract, article test, and cross-repo validators synchronized after any future input-bundle protocol change. |
| HE-7 latest-forecast-only protocol | `done` | Manuscript `332-336`; response HE-7; article `artifacts/latest_forecast_issue/latest_forecast_issue_manifest.json`; workflow `docs/he7_latest_forecast_issue_contract_20260615.md`; figures `fig:ensembles` and cutoff support panels | Response and manuscript now state the source-specific latest-only rule: GloFAS uses the cutoff daily issue, NWS uses the most recent available issuance per target/member before daily aggregation, and older issuances are not averaged into publication matrices. | Completed with HE-7 contract tests, manifest code-evidence path checks, cross-repo validators, and a 10/10 runtime alias byte-identity audit; legacy `weighted_daily` names are documented as compatibility aliases. |

### Reviewer 1 Overview And Major Comments

| ID | Current status | Evidence / manuscript anchors | Audit question | Confirmation gate |
|---|---|---|---|---|
| R1-O overview / forecasting emphasis | `done` | Response overview `main.tex` around R1-O; manuscript Introduction `68-80`, Forecast validation `347-370`, Interpretation `372-424`, Conclusions `429-431`; workflow `docs/r1_overview_forecasting_emphasis_contract_20260615.md`; article `docs/reviewer1_overview_forecasting_emphasis_contract.md` | Response now says the manuscript has been reframed from model-development emphasis toward rolling-origin forecast validation, CRPS and quantile diagnostics, horizon-aware raw-product comparisons, and selected-model interpretation. | Completed with a named R1-O contract, article tests, publication-freeze gates, cross-repo `reviewer1_overview` audit output, and stale main-article framing guards. |
| R1-M1 meteorological vs hydrological uncertainty | `done` | Response R1-M1 `main.tex` around `308`; manuscript Introduction `70-78`, External Data Sources `264`; workflow reviewer notes `docs/reviewer_refs/R1_M1_uncertainty_notes.md`; workflow `docs/r1_m1_uncertainty_framing_contract_20260615.md`; article `docs/reviewer1_uncertainty_framing_contract.md` | The response now states that the revised introduction separates hydrological uncertainty from meteorological/input uncertainty before introducing the Bayesian framework. The data-source prose now uses hydrometeorological covariate terminology for precipitation and soil-moisture transfer inputs. | Completed with a named R1-M1 uncertainty-framing contract, article tests, publication-freeze gates, cross-repo `reviewer1_uncertainty` audit output, and stale wording guards for the former purely hydrological covariate language. |
...
| R1-M3 reduce mathematical detail/PIT/quantile crossing | `done` | Response R1-M3; manuscript CRPS selection `228-240`, discount-factor robustness note `330`, synthesis illustration `414-424`, MCMC/VB appendices `558+` and `664+`; article checklist Phase B | The revised manuscript removes the detailed PIT development from the main text, uses CRPS and targeted quantile check loss as the reported forecast-validation criteria, keeps posterior predictive synthesis as a concise selected-origin illustration, and limits quantile crossing to a single robustness note in the discount-factor discussion. | Completed with R1-M3 prose gates in the article tests and workflow validators; implementation-level algorithms remain in the appendices for reproducibility rather than in the main narrative. |
| R1-M4 expanded forecast evidence beyond one event | `done` | Manuscript Rolling-origin design `330-334`, Forecast validation `345-377`, synthesis illustration `417-430`; response R1-M4 | The response and article now state that the original single-window emphasis was replaced by a five-cutoff rolling-origin evaluation with held-out USGS verification, CRPS, and targeted quantile diagnostics. The December 2022 synthesis and component figures are explicitly illustrative/diagnostic and are not counted as additional validation evidence. | Closed with prose gates in the article tests and workflow validators. The revised article remains explicit that this is not a continuous daily 2023-present hindcast. |
| R1-M5 cross-validation/fair forecast assessment | `done` | Manuscript Rolling-origin design `330-334`; response R1-M5; forecast-design manifest and HE-6/R1-M5 contract | The revised article and response now frame the assessment as five cutoff-specific rolling-origin forecast-window evaluations. Each retained origin defines a version-consistent staged dataset, uses only information available at that origin to fit seven quantile-specific models and synthesize the posterior predictive distribution, and verifies against held-out post-cutoff USGS observations. | Closed with article/manifest tests and workflow validators. The article and response also state that the retained origins are constrained by version-consistent forecast archives, should not be represented as a dense continuous post-2022 hindcast, and evaluate a fixed calibrated specification after preliminary pre-cutoff calibration of \(\lambda\). |

### Reviewer 1 Minor Comments

| ID | Current status | Evidence / manuscript anchors | Audit question | Confirmation gate |
|---|---|---|---|---|
| R1-m1 conceptual vs physically based hydrological models | `done` | Manuscript Introduction `68-80`; response R1-m1 | The revised introduction now states that operational hydrological forecasts are commonly produced with conceptual or physically based models and adds that conceptual formulations are often practical for prediction because their lower-dimensional structure is easier to specify, calibrate, and deploy operationally. | Closed with article unit-test coverage and publication-freeze/cross-repo prose gates; response language is now completed-action rather than future tense. |
| R1-m2 “flexile” typo | `done` | Manuscript full text search; response R1-m2; article Section 2.1 heading; article checklist `docs/manuscript_revision_checklist.md` | The revised manuscript no longer contains the typo and the former wording has been replaced by the corrected exAL likelihood subsection terminology. The response now uses completed-action wording. | Closed with article unit-test coverage and publication-freeze/cross-repo article-scoped prose gates. The forbidden-string check is intentionally scoped to the revised article because the reviewer comment quotes the original typo. |
| R1-m3 ERA5/reanalysis uncertainty | `done` | Manuscript External Data Sources `264`; response R1-m3; article checklist `docs/manuscript_revision_checklist.md` | The revised manuscript now states that ERA5-Land inputs are reanalysis-based model inputs rather than direct observations or uncertainty-free measurements, and notes that ERA5/ERA5-Land variables may include short forecast components. The response also clarifies the application-specific split between PRISM precipitation and ERA5-Land soil moisture. | Closed with article unit-test coverage and publication-freeze/cross-repo prose gates. The gates are scoped to the ERA5/reanalysis uncertainty claim and intentionally do not ban valid technical uses of `deterministic`. |
| R1-m4 introduce USGS and separate data/methodology | `done` | Manuscript Application Data and Forecasting Design `242-336`; Study Setting and Observations `245-260`; External Data Sources `262-285`; response R1-m4 | The revised manuscript now separates methodology from application data and forecasting design, begins the application section with the Big Trees USGS daily flow series as the observed target, then distinguishes forecast covariates, retrospective products, and operational forecast products before the rolling-origin design. | Closed with article unit-test coverage and publication-freeze/cross-repo prose gates preserving the USGS-target versus retrospective-input distinction and the data/methodology separation. |
| R1-m5 precipitation zeros/censoring | `done` | Manuscript External Data Sources and Forecast Covariates `262-270`; forecast-design manifest; article forecast-design contract; response R1-m5 | The revised manuscript now states explicitly that precipitation is not handled through censoring, zero-inflation, or a separate occurrence/intensity model. Zero-precipitation days are retained in the supplied covariate path, and precipitation intermittency enters through the transfer component and deterministic engineered terms. | Closed with article unit-test coverage, covariate-feature zero-retention regression, and publication-freeze/cross-repo prose gates. |
...
| R1-m7 vague “General Results” / separate checking vs forecasting | `done` | Manuscript `Forecast Validation Results` section, `Interpretation of the Selected Specification` section, and R1-m7 response | The revised manuscript no longer uses the vague `General Results` organization. Forecast validation, application setup, and selected-model interpretation are separated, and interpretive figures are explicitly not presented as additional forecast-validation evidence. | Closed with completed-action response language plus article, publication-freeze, and cross-repo section-separation gates. |
...
| R1-m9 figures explanation / constant variance vs variable predictive distributions | `done` | Manuscript selected-model dynamics and synthesis illustration; long-cycle appendix; univariate transfer-active synthesis appendix; response R1-m9 | The response and manuscript now distinguish fitted quantile-location/component uncertainty from the full synthesized posterior predictive distribution. Stable bands in the diagnostic figures are not presented as full forecast predictive distributions, while the synthesis envelope is allowed to vary with source corrections and forecast-window inputs. | Closed with article manifest/prose tests plus publication-freeze and cross-repo prose gates. |
