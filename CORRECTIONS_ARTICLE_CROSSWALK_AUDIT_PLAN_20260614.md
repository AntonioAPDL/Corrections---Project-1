...
| R1-M5 cross-validation/fair forecast assessment | `done` | Manuscript Rolling-origin design `330-334`; response R1-M5; forecast-design manifest and HE-6/R1-M5 contract | The revised article and response now state that rolling-origin folds are the time-ordered analogue of cross-validation for the operational forecasting problem. Each fold fixes a forecast origin, uses only information available at that origin, and verifies against future USGS observations. | Closed with article/manifest tests and workflow validators. The article and response also state that the retained folds are constrained by version-consistent forecast archives and should not be represented as a dense continuous post-2022 hindcast. |
