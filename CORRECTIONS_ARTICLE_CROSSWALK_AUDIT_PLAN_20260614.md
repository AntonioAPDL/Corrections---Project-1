...
| HE-2 validation against alternatives/raw products/exAL vs AL | `likely_complete` | Manuscript `348-363`; tables `tab:benchmark_crps_models`, `tab:benchmark_crps_models_nws_horizon`; response HE-2 generated table | Check all prose matches horizon split: 28-day excludes NWS; 8-day table includes NWS; `N` rows are normal DLM baselines, not quantile lanes. | Confirm response should reference both HE2 tables and not say raw NWS is in the 28-day table. |
| HE-3 ablation study | `likely_complete` | Manuscript `368-377`; tables `tab:he3_ablation_crps`, `tab:he3_ablation_crps_nws_horizon`; response HE-3 generated tables | Check response and manuscript both state full model is best under both horizon contracts and define `noH3` as the noninteger third harmonic. | Confirm whether ablation tables stay in main text or move to supplement/appendix with response references. |
| HE-4 CRPS and quantile diagnostics | `likely_complete` | Manuscript `348-365`; table `tab:he4_quantile_check_loss`; response HE-4 | Check CRPS is described as forecast verification, not only model selection; check quantile check-loss table is referenced clearly. | Confirm no PIT analysis is retained and check-loss is the intended quantile diagnostic. |
| HE-5 code/reproducibility | `needs_decision` | Manuscript code availability `451-455`; response HE-5 | Response promises a permanent archived release before resubmission; manuscript currently cites GitHub/CRAN but not necessarily a DOI/release. | Decide whether to add a Zenodo/OSF/archival-release placeholder now or wait until release exists. |
| HE-6 out-of-sample validation | `likely_complete` | Manuscript `330-334`; response HE-6 | Check the distinction among fit-time inputs, forecast-time inputs, and post-cutoff USGS verification is precise. | Confirm wording for forecast covariates: forecast PPT/SOIL after cutoff, GDPC/PCA not forecast the same way. |
| HE-7 latest-forecast-only protocol | `likely_complete` | Manuscript `332-334`; response HE-7; figures `fig:ensembles` and cutoff support panels | Check response states old weighted multi-issuance protocol was replaced; manuscript says latest daily issue retained at each cutoff. | Confirm no remaining wording implies lagged forecast averaging. |

### Reviewer 1 Overview And Major Comments

| ID | Current status | Evidence / manuscript anchors | Audit question | Confirmation gate |
|---|---|---|---|---|
| R1-O overview / forecasting emphasis | `needs_polish` | Response overview `main.tex` around R1-O; manuscript Introduction `68-80`, Forecast validation `345-377` | Check the response no longer says “will” for changes already implemented, unless intentionally future-facing. | Confirm tone: concise bridge from old model-development emphasis to revised validation-centered paper. |
| R1-M1 meteorological vs hydrological uncertainty | `likely_complete` | Manuscript Introduction `68-78`; response R1-M1 | Check response points to the exact rewritten introduction and avoids overclaiming. | Confirm if one additional sentence is needed in response saying this is now in the opening paragraph. |
| R1-M2 link model formulation to results | `needs_polish` | Manuscript Methodology `82-239`, Forecast validation `345-377`, Interpretation `378-430`; response R1-M2 | Response currently says “will”; manuscript now has a clearer section structure. | Convert response to completed-action language and cite specific sections/subsections. |
...
| R1-m6 latest forecast vs lagged forecasts | `likely_complete` | Manuscript `332-334`; response R1-m6 | Same substance as HE-7. | Confirm response can cross-reference HE-7 to avoid repetition. |
